...
export default function App() {
  ...
  useEffect(() => {
    let active = true;
    setDocument(null); setError(null); setLoading(true); setPlaying(false); setHour(0); setSelected([]);
  ...
  return <main className="app-shell" data-time={environment?.timeOfDay} data-season={environment?.season} data-motion={settings.motion}>
    <header className="site-header"><a className="brand" href="./" aria-label="OnlyFriends home"><span className="brand-symbol"><i /><i /><i /></span>onlyfriends<span className="brand-dot">®</span></a><nav className="view-tabs" aria-label="Main navigation">{(['overview', 'turbines'] as const).map(next => <a key={next} href={`#${next}`} aria-current={view === next ? 'page' : undefined} onClick={event => { event.preventDefault(); navigate(next); }}>{next === 'overview' ? 'Overview' : 'Turbines'}</a>)}</nav><div className="header-right"><span className="demo-indicator"><i /> {mode === 'historical' ? 'HISTORICAL REPLAY' : 'LIVE FORECAST'}</span><button className="avatar" aria-label="Open display settings" onClick={() => setSettingsOpen(value => !value)}>OF</button></div></header>
