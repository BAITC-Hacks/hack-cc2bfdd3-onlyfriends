...
export function AssistantBar({ runId, selectedLeadHour, selectedTurbineId, horizon, selectedAt }: AssistantBarProps) {
  ...
  return <div className="assistant-wrap">
    ...
    <div className="suggestions"><span>Например</span>{['Почему сегодня сильный ветер?', 'Когда пик мощности?'].map(question => <button key={question} disabled={loading} onClick={() => { setQuery(question); void ask(question); }}>{question}<Icon name="diagonal" size={12} /></button>)}</div>
  </div>;
}
