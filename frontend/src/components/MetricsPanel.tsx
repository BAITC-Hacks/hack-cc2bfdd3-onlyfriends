...
interface Props { hour: ForecastHour; selected: string[]; onSelect: (ids: string[]) => void }
export function MetricsPanel({ hour, selected, onSelect }: Props) {
  ...
  return <>
    <aside className="metrics-strip" aria-label="Farm forecast metrics">
      <div><span>{selected.length ? `${selected.length} selected · Predicted output` : 'Predicted output'}</span><strong data-testid="forecast-power">{summary.power.toFixed(1)}<small>MW</small></strong></div>
      <div><span>Wind</span><strong>{summary.windSpeed.toFixed(1)}<small>m/s</small></strong></div>
    ...
    {reading && turbine && <aside className="turbine-insight" aria-label="Turbine insight">
      ...
      <h2>{turbine.name}</h2>
      <div className="insight-values"><strong>{reading.power.toFixed(1)}<small> MW</small></strong><span><Icon name="wind" size={12} />{reading.windSpeed.toFixed(1)} m/s</span></div>
      <p>{Math.round(reading.power / 3.6 * 100)}% capacity · {Math.round(reading.temperature)}°C · {reading.direction}°</p>
    </aside>}
