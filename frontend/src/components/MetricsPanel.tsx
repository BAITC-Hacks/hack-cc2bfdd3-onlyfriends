...
export function MetricsPanel({ hour, selected, onSelect }: Props) {
  ...
  return <>
    <aside className="metrics-strip" aria-label="Farm forecast metrics">
      <div><span>{selected.length ? `${selected.length} selected · Normalized output` : 'Normalized output'}</span><strong data-testid="forecast-power">{summary.power.toFixed(3)}<small>/ {selected.length === 1 ? '1' : '2'}</small></strong></div>
