...
interface Props { hour: ForecastHour; turbines: TurbineData[]; selected: string[]; onSelect: (id: string) => void; environment: ForecastEnvironment; reset: number; zoom: number; onZoom: (zoom: number) => void; settings: DisplaySettings; wind: WindAt }
export default function PlanetScene({ hour, turbines, selected, onSelect, environment, reset, zoom, onZoom, settings, wind }: Props) {
  return <SceneBoundary key={String(settings.suppliedModel)}><Canvas shadows dpr={[1, 1.75]} camera={{ position: [0, 1.3, sceneConfig.cameraDistance], fov: 38 }} gl={{ antialias: true, alpha: true }} fallback={<div className="scene-fallback">WebGL is unavailable. Explore your forecast with the controls below.</div>}>
    ...
    <Suspense fallback={null}><SeasonalEnvironment season={environment.season} motion={settings.motion}><PlanetInteraction motion={settings.motion} reset={reset} zoom={zoom} onZoom={onZoom} landscape={<Landscape hour={hour} turbines={turbines} selected={selected} onSelect={onSelect} motion={settings.motion} showLabels={zoom > 3.6} wind={wind} />}>
      <Planet />
