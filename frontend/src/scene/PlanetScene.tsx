...
interface Props { hour: ForecastHour; turbines: TurbineData[]; selected: string | null; onSelect: (id: string) => void; zoom: 1 | 2; reset: number; settings: DisplaySettings }
export default function PlanetScene({ hour, turbines, selected, onSelect, zoom, reset, settings }: Props) {
  ...
  return <SceneBoundary key={String(settings.suppliedModel)}><Canvas shadows dpr={[1, 1.75]} camera={{ position: [0, 1.3, sceneConfig.cameraDistance], fov: 38 }} gl={{ antialias: true, alpha: true }} fallback={<div className="scene-fallback">WebGL is unavailable. Explore your forecast with the controls below.</div>}>
    ...
    <Suspense fallback={null}><PlanetInteraction motion={settings.motion} reset={reset} focus={focus}>
      <Planet />
      {turbines.map((turbine, index) => <Turbine key={turbine.id} turbine={turbine} reading={hour.readings.find(r => r.turbineId === turbine.id)!} displayOffset={(index - (turbines.length - 1) / 2) * 0.22} selected={selected === turbine.id} onSelect={onSelect} {...settings} />)}
