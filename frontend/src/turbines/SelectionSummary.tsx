...
export function SelectionSummary({ hour, selected }: { hour: ForecastHour; selected: string[] }) {
  ...
  return <section className="selection-summary" aria-label="Selected turbine summary">
    ...
    {subset.readings.length === 0 ? <p className="summary-empty">Select turbines to compare their forecast.</p> : mode === 'aggregate' ? <div className="summary-values">
      <div><strong>{metrics.power.toFixed(3)} / {selected.length}</strong><span>normalized output</span></div>
    ...
    </div> : <div className="individual-values" tabIndex={0} aria-label="Individual selected forecasts">{subset.readings.map(reading => <div key={reading.turbineId}><strong>{reading.turbineId}</strong><span>{reading.power.toFixed(3)} / 1</span><small>{reading.windSpeed.toFixed(1)} m/s · {Math.round(reading.temperature)}°C</small></div>)}</div>}
