...
export function SelectionSummary({ hour, selected }: { hour: ForecastHour; selected: string[] }) {
  ...
  const metrics = subset.readings.length ? summarize(subset) : null;
  return <section className="selection-summary" aria-label="Selected turbine summary">
    ...
    {!metrics ? <p className="summary-empty">Select turbines to compare their forecast.</p> : mode === 'aggregate' ? <div className="summary-values">
      <div><strong>{(metrics.power * 100).toFixed(1)}%</strong><span>mean normalized power</span></div>
    ...
    </div> : <div className="individual-values" tabIndex={0} aria-label="Individual selected forecasts">{subset.readings.map(reading => <div key={reading.turbineId}><strong>{reading.turbineId}</strong><span>{(reading.power * 100).toFixed(1)}%</span><small>{reading.windSpeed.toFixed(1)} m/s · {Math.round(reading.temperature)}°C</small></div>)}</div>}
