...
export function TurbineTable({ rows, timestamp, mode, selected, onToggle, onSelectPage, onReset }: Props) {
  ...
  return <div className="turbine-table-scroll" tabIndex={0} role="region" aria-label="Turbine forecast table">
    <table className="turbine-table"><caption className="sr-only">Turbine forecasts for {hourLabel(timestamp, true)}, UTC+5</caption>
      <thead><tr><th scope="col"><input ref={checkbox} type="checkbox" aria-label="Select visible turbines" checked={rows.length > 0 && count === rows.length} disabled={!rows.length} onChange={e => onSelectPage(e.target.checked)} /></th><th scope="col">Turbine</th><th scope="col">Data</th><th scope="col">Wind 100 m</th><th scope="col">Direction</th><th scope="col">Normalized power</th><th scope="col">Temperature</th><th scope="col">Pressure</th><th scope="col">Forecast time</th></tr></thead>
      <tbody>{rows.map(row => {
        ...
        return <tr key={row.id} data-selected={selected.includes(row.id)}>
          ...
          <td>{reading ? `${reading.pressure.toFixed(1)} hPa` : '—'}</td>
