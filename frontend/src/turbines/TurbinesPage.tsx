...
import type { ForecastHour, Turbine } from '../forecast/forecast';
...
interface Props { hour: ForecastHour; turbines: Turbine[]; mode: 'historical' | 'live'; selected: string[]; onSelection: (selected: string[]) => void; onView3D: () => void }
export function TurbinesPage({ hour, turbines, mode, selected, onSelection, onView3D }: Props) {
  ...
  return <section className="turbines-page" aria-labelledby="turbines-heading">
    ...
    <TurbineTable rows={items} timestamp={hour.at} mode={mode} selected={selected} onToggle={toggle} onSelectPage={selectPage} onReset={() => changeFilters(DEFAULT_FILTERS)} />
